...
public class NaviItemDecorator implements IAttributeDecorator{
	...
	static {
		try {
          ...
          LoggerFactory.getLogger(NaviItemDecorator.class).error(MarkerFactory.getMarker("FATAL"), "IASSiteDataService  init failure", e);
		}
	}
		...
		try{
			...
			if (doc instanceof Site) {
				href = doc.getDefinedParentName().toLowerCase() + "Site" + StringUtils.capitalize(attributeName) + "Show?pId="
						+ doc.getId() +"&ownerId="+doc.getId()+ "&ts=" + System.currentTimeMillis();
